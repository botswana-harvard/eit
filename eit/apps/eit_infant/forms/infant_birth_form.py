    ...
    def clean(self):
        ...
        if MaternalPostReg.objects.filter(registered_subject__subject_identifier=cleaned_data.get('registered_subject').relative_identifier):
            ...
            if not cleaned_data.get('dob', None) == delivery.delivery_datetime.date():
                raise forms.ValidationError(
                    'Infant dob must match maternal delivery date of %s. You wrote %s' % (delivery.delivery_datetime.date(), cleaned_data.get('dob', None),))
            if cleaned_data.get('infant_redcap_sbid'):
                ...
                match = re.search('S-\d{2}-\d{4}-\d{2}',
                                  cleaned_data.get('infant_redcap_sbid'))
                if not match:
                    raise forms.ValidationError(
                        'The Screening REDCAP Bid you provided does not conform to '
                        'the presribed format for Screening Bids.')
        if self.sbid_check():
